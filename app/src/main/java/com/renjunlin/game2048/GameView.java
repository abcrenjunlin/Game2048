...
public class GameView extends GridLayout {
    ...
    private void addCards(int cardsWidth,int cardsHeight){
        for(int y=0;y<4;y++){
            for(int x=0;x<4;x++){
                Card c=new Card(getContext());
                c.setNum(0);
                addView(c,cardsWidth,cardsHeight);
                cardsMap[x][y]=c;
            }
        }
    }
    ...
    private void checkComplete(){
        ...
        if(complete){
            new AlertDialog.Builder(getContext()).setTitle("你好").setMessage("游戏结束").setPositiveButton("从来", new DialogInterface.OnClickListener() {
                @Override
